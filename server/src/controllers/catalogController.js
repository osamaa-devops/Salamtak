...
export const patients = asyncHandler(async (req, res) => {
  res.json({ success: true, data: await service.listPatients(req.query) });
});
...
export const updateMedicationSchedule = asyncHandler(async (req, res) => {
  res.json({ success: true, data: await service.updateMedicationSchedule(req.params.id, req.body) });
});

export const deleteMedicationSchedule = asyncHandler(async (req, res) => {
  await service.deleteMedicationSchedule(req.params.id);
  res.status(204).send();
});
...
export const reviews = asyncHandler(async (req, res) => {
  const [items, stats] = await Promise.all([
    ...
    service.getReviewStats(req.query.category),
  ]);
...
export const markReviewHelpful = asyncHandler(async (req, res) => {
  res.json({ success: true, data: await service.markReviewHelpful(req.params.id) });
});
